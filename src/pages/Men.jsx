...
const Men = ()=>{
    ...
    const FilterProducts = (z) =>{
      console.log("ban dau : ")
      
      let newdata = []
      DataProducts.map((p) =>{
            if(p.id == 1)console.log(p.listColor)
            if(p.tags.includes(z)){
                if(p.id == 1)console.log(p.listColor)
                newdata.push(p)
            }
      })
    ...
    return(
        <div  className='menpage'>
            ...
            {showFilter? <FilterPrduct page="Men" DataProducts ={FilterProducts}/> : null}
            <MainProducts page="Men" statusFilter={showFilter} showFilter={changeShowFilter} DataProducts = {[...presentProducts]}/>
            </div>
